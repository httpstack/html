...
echo $p;
//$content = $fl->readFile($p);
//dd($p);
//$fileContent = file_get_contents($p);
$viewNode = $this->toDomObject($p);
...
$strXpath = "//*[@data-key='view']";
//$targetNode = $this->template->map->query($strXpath)->item(0);
$targetNode = $this->template->getElementById('view'); // Assuming 'view' is the ID of the target node

if (!$targetNode) {
    throw new \Exception("Target node for view not found in template.");
}
